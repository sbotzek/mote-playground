;;;; Copied from mote and modified for our purpose
(ns experiments.tmap-combine-buffers-tmap
  (:require
   [mote.gl :as mgl]
  ...
  (go
    (let [response (<p! (js/fetch path))
          ...
          objects-buffer (-> gl
                             ...
                             (mgl/add-buffer-attribute 0 2)
                             (mgl/add-buffer-attribute 1 4)
                             (mgl/add-buffer-data (mapcat #(get-in % [:buffer :data]) (filter #(= (:type %) :objects) layers)))
                             (mgl/sync-buffer-data))
          tiles-buffer (-> gl
                           (mgl/create-buffer :draw-mode :triangles)
                           (mgl/add-buffer-attribute 0 2)
                           (mgl/add-buffer-attribute 1 2)
                           (mgl/add-buffer-attribute 2 1)
                           (mgl/add-buffer-data (mapcat #(get-in % [:buffer :data]) (filter #(= (:type %) :tiles) layers)))
                           (mgl/sync-buffer-data))
          tilemap {:gl gl
                   :width (aget json "width")
                   :height (aget json "height")
                   ...
                   :buffers {:tiles tiles-buffer :objects objects-buffer}}]
      (println (get-in tilemap [:buffers :tiles]))
      tilemap)))
...
(defn draw
  ...
  (let [gl (:gl tilemap)
    ...
    ;; draw the tiles
    (let [shader (:tiles shaders)
          buffer (:tiles buffers)]
