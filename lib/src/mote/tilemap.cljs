...
(defn load-tiled
  ...
  [gl path]
  (go
    (let [json (<! (assetdb/load-asset path))
          tilesets (<! (go-loop [[json & rest] (aget json "tilesets")
                                 sofar []]
                         (let [sofar' (conj sofar (<! (json->tileset gl (base-path path) json)))]
                           (if (seq rest)
                             (recur rest sofar')
                             sofar'))))
          ...
          tiles-shader-source (<! (assetdb/load-asset "/glsl/tilemap-tiles.glsl"))
          tiles-shader (mgl/load-shader gl tiles-shader-source)
          objects-shader-source (<! (assetdb/load-asset "/glsl/tilemap-objects.glsl"))
